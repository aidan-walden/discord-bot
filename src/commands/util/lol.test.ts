...
function buildInteraction(opts: BuildOpts = {}): ChatInputCommandInteraction {
	...
	const riot = {
		...
		getAccountByPuuid: mock(async () => ({
			puuid: "p1",
			gameName: "Faker",
			tagLine: "KR1",
		})),
		getLeagueEntriesByPuuid: mock(async () => [
			{
		...
		getActiveGame: mock(async () => null),
		getMatchIdsByPuuid: mock(async () => ["m1"]),
		getMatch: mock(async () => ({
		...
		getSummonerByPuuid: mock(async () => ({
			puuid: "p1",
			profileIconId: 1,
			summonerLevel: 100,
		})),
		getRankHistory: mock(async () => []),
	};
